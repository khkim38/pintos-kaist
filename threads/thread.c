					...
					thread_func *function, void *aux)
{
	...
	// t->file_list = (struct file **)malloc(sizeof(struct file *) * 128);
...
void thread_set_priority(int new_priority)
{
	...
	if (!list_empty(&curr->donation_list))
	{
		for (e = list_begin(&curr->donation_list); e != list_end(&curr->donation_list); e = list_next(e))
		{
			...
			if (temp->priority > highest_prioirty)
				highest_prioirty = temp->priority;
		}
...
init_thread(struct thread *t, const char *name, int priority)
{
	...
	sema_init(&t->fork_child_semaphore, 0);
